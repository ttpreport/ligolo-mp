...
func (aah *AgentApiHandler) serve(protocol string, listenIface string, tlsConfig *tls.Config) {
	...
	redirector, err := tls.Listen(protocol, listenIface, tlsConfig)
	if err != nil {
		panic(err)
	}
	defer redirector.Close()

	slog.Info("operator handler started",
		slog.Any("address", listenIface),
	)

	for {
		conn, err := redirector.Accept()
		if err != nil {
			slog.Error("Redirector encountered an error",
				slog.Any("error", err),
			)
